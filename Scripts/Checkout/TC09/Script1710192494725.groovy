...
WebUI.setText(findTestObject('Object Repository/Checkout/FullName_Checkoutpag'), generateRandomchar(8))

WebUI.setText(findTestObject('Object Repository/Checkout/Email_CheckoutPage'), generateRandomEmail())
...
WebUI.setText(findTestObject('Object Repository/Checkout/Address_CheckoutPage'), 'no 56/34 bv nager')
...
String Get_URL_checkout = trimRandomPart(WebUI.getUrl())
...
if (Get_URL_checkout != Get_URL_if_Checkout) {
	KeywordUtil.markFailed("The Fullname accepts special characters")
...
while (x == true) {
	Untilcarturl = WebUI.getUrl()

	if (Untilcarturl.contains('cart')) {
		x = false
	} else {
		WebUI.back()
	}
